...
export class FlipService {
  ...
  updateStateData(districtData: DistrictVotes[]) {
    ...
    districtData.forEach((district) => {
      ...
      if (
        district.dVotes >= district.rVotes &&
        district.dVotes >= district.oVotes
      ) {
        dDistricts += 1;
      } else if (
        district.rVotes >= district.dVotes &&
        district.rVotes >= district.oVotes
      ) {
        rDistricts += 1;
      } else if (
        district.oVotes >= district.dVotes &&
        district.oVotes >= district.rVotes
      ) {
        oDistricts += 1;
      }
    });
  ...
  updateGrid() {
    const cutDGrid =
      this.stateData.dPercent * FLIPGRIDLAYOUT.rows * FLIPGRIDLAYOUT.cols;
    const cutOtherGrid =
      ((this.stateData.dVotes + this.stateData.oVotes) /
        this.stateData.totalVotes) *
      FLIPGRIDLAYOUT.rows *
      FLIPGRIDLAYOUT.cols;
    ...
    select(this.elId)
      ...
      .style('fill', (d, i, nodes: HTMLElement[]) =>
        nodes[i].getAttribute('squareColor')
      );
  }
    ...
    select(this.elId)
      ...
      .filter(
        (d, i, nodes: HTMLElement[]) =>
          +nodes[i].getAttribute('district') == bar.data.district
      )
      .raise()
  ...
  updateDisplay(visContainer: HTMLElement) {
    ...
    select(visContainer)
      ...
      .duration(100)
      .style('fill', (d, i, nodes: HTMLElement[]) =>
        this.getCongressSquareColor(nodes[i])
      );
  }
  ...
  styleMapOnHover(
    ...
    const districts = select(container).selectAll('.district-boundary');
    const activeDistrictSel = districts.filter(
      (d, i, nodes: HTMLElement[]) =>
        +nodes[i].getAttribute('district') === activeDistrict
    );
    const inactiveDistrictsSel = districts.filter(
      (d, i, nodes: HTMLElement[]) =>
        +nodes[i].getAttribute('district') !== activeDistrict
    );

    activeDistrictSel.classed('active-mouseover', action === 'start').raise();
    inactiveDistrictsSel.classed('inactive-mouseover', action === 'start');
  }
}
