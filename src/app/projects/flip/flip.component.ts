...
export class FlipComponent implements OnInit {
    ...
    makeVoteDisplay(svg) {
        ...
        var repVotesDisplay = displaySvg.append('g');
    ...
    makeBars(svg) {
        ...
        const barsBoxWidth =
            numBars * (barMargin.left + FLIPBARLAYOUT.width + barMargin.right) + (boxMargin.left + boxMargin.right);
        ...
        const barSvg = barsContainer
            ...
            .attr('x', function (d, i) {
                return boxMargin.left + i * (FLIPBARLAYOUT.width + barMargin.right + barMargin.left);
            })
            .attr('y', barOffset);
    ...
    makeLowerVis(svg) {
        ...
        const container = svg.append('svg').attr('class', 'container').attr('width', width).attr('height', height);
        ...
        districtInfoContainer.append('text').attr('class', 'district-info district-name');
        ...
        districtInfoContainer.append('text').attr('class', 'district-info total-votes').style('text-anchor', 'end');

        districtInfoContainer.append('text').attr('class', 'district-info cand-name dem');

        districtInfoContainer.append('text').attr('class', 'district-info cand-votes dem').style('text-anchor', 'end');

        districtInfoContainer.append('text').attr('class', 'district-info cand-name rep');

        districtInfoContainer.append('text').attr('class', 'district-info cand-votes rep').style('text-anchor', 'end');

        districtInfoContainer.append('text').attr('class', 'district-info cand-name oth');

        districtInfoContainer.append('text').attr('class', 'district-info cand-votes oth').style('text-anchor', 'end');
    ...
    makeStateVotes(svg) {
        ...
        const cutOtherGrid = (this.flipService.stateData.dPercent + this.flipService.stateData.oPercent) * rows * cols;
            ...
            .delay((d, i, nodes) => {
                if (nodes[i].getAttribute('squareColor') == FLIPCOLORS.dColor) {
                    ...
                    if (cutOtherGrid < numSquares / 2) {
                        return pause + (numSquares - nodes[i].getAttribute('cellnum') + 1) * delay + 100 * delay;
                    } else {
                        return pause + (nodes[i].getAttribute('cellnum') + 1) * delay + 100 * delay;
                    }
                } else if (nodes[i].getAttribute('squareColor') == FLIPCOLORS.rColor) {
                    return pause + (numSquares - nodes[i].getAttribute('cellnum') + 1) * delay;
                }
            })
    ...
    makeCongressSeats(svg) {
        ...
        congressSvg
            ...
            .style('stroke-linecap', 'butt')
            .attr('squareColor', (d, i, nodes) => this.flipService.getCongressSquareColor(nodes[i]))
            .transition()
            .delay((d, i, nodes) => {
            ...
            .style('fill', (d, i, nodes) => this.flipService.getCongressSquareColor(nodes[i]));
    }
