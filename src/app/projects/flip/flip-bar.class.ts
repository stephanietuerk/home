...
export class FlipBar {
    ...
    constructor(data: DistrictVotes, el: HTMLElement, visContainer: HTMLElement, flipService: FlipService) {
        ...
        this.cutOther = (this.data.dPercent + this.data.oPercent) * FLIPBARLAYOUT.rows * FLIPBARLAYOUT.cols;
        this.selectSvg();
    ...
    selectSvg() {
        this.svg = select(this.el).attr('class', 'bar').attr('district', this.data.district).style('fill', 'none');
    }
    ...
    makeSquares() {
        this.svg
            ...
            .attr('y', (j) => FLIPBARLAYOUT.squareSize * Math.floor(j / FLIPBARLAYOUT.cols))
            ...
            .attr('squareColor', (d, j, n) => {
                ...
                } else if (el.getAttribute('cellnum') > this.cutD && el.getAttribute('cellnum') <= this.cutOther) {
            ...
            .delay((d, j, n) => {
                ...
                if (el.getAttribute('squareColor') == FLIPCOLORS.dColor) {
                    return FLIPTRANSITIONS.pause + el.getAttribute('cellnum') * FLIPTRANSITIONS.delay;
                    ...
                    return (
                        ...
                        (this.cutD * FLIPTRANSITIONS.delay +
                            (FLIPBARLAYOUT.numSquares - el.getAttribute('cellnum') + 1) * FLIPTRANSITIONS.delay)
                    );
                }
            })
            .style('fill', (d, j, n) => {
    ...
    getVotesFromPosition(x: number, y: number, data: DistrictVotes): number {
        const barToVotes = scaleLinear()
            .domain([0, Math.sqrt(Math.pow(FLIPBARLAYOUT.height, 2) + Math.pow(FLIPBARLAYOUT.width, 2))])
    ...
    getStartParty(onClickVotes: number): void {
        ...
        } else if (onClickVotes > this.dragStartVotes.rVotes + this.dragStartVotes.oVotes) {
            this.dragStartParty = Party.D;
        }
    }
    ...
    updateDraggedData(dragVotes: number) {
        ...
        let rVotes;
        let oVotes;
        const startRVotes = this.data.totalVotes - (this.dragStartVotes.dVotes + this.dragStartVotes.oVotes);
        ...
        if (this.dragStartParty == Party.D) {
            ...
            if (dragVotes < this.dragStartVotes.rVotes) {
                ...
                rVotes = dragVotes;
                oVotes = this.data.totalVotes - dVotes - rVotes;
            } else if (dragVotes >= this.dragStartVotes.rVotes && dragVotes < this.dragStartVotes.dVotes) {
                dVotes = this.dragStartVotes.dVotes;
                rVotes = this.dragStartVotes.rVotes;
    ...
    updateBar(): void {
        const cutDBar = this.draggedData.dPercent * FLIPBARLAYOUT.rows * FLIPBARLAYOUT.cols;
        const cutRBar =
            (this.draggedData.dPercent + this.draggedData.oPercent) * FLIPBARLAYOUT.rows * FLIPBARLAYOUT.cols;
