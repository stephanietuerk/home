...
export class SummaryComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        const sort$ = this.sort.pipe(
            map(() => (vm: ViewModel) => {
                ...
                return { ...vm, tableData, chartConfig: this.getUpdatedChartConfigForSort(vm.chartConfig, chartSort) };
            })
        );
    ...
    getTableData(data: JobDatum[]): JobTableDatum[] {
        ...
        const tableData = fields.map((field) => {
            ...
            const currentValue = fieldData.find((x) => x.year.getFullYear() === currentYear).count;
            return {
                field,
                avg,
