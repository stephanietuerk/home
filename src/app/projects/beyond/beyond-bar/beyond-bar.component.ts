...
export class BeyondBarComponent implements OnInit, OnChanges {
  ...
  setData() {
    this.data = Object.entries(this.beyondService.tractData).map(
      ([k, v]: [string, TractDatum]) => {
  ...
  makeBarMain(): void {
    this.svgMain = select(this.divId)
      ...
      .attr(
        'width',
        this.width.main + this.margin.main.left + this.margin.main.right
      )
  ...
  makeAxisLabels(): void {
    ...
    this.barMain
      ...
      .call(wrap, {
        width: this.margin.main.left - this.axisOffset,
        wrapUp: true,
      });
  ...
  makeBarIndex(): void {
    this.svgIndex = select(this.divId)
      ...
      .attr(
        'width',
        this.width.index + this.margin.index.left + this.margin.index.right
      )
      .attr('height', this.height + this.margin.top + this.margin.bottom)
  ...
  makeBrush(): void {
    ...
    this.svgIndex
      .selectAll('.overlay')
      .on('mousedown touchstart', this.handleIndexBarClick.bind(this));
  }
  ...
  getVoteColor(x: { string: TractDatum }): string {
    ...
    if (value === 0) {
      ...
      const scaleType =
        this.electionYear === ElectionYear.change ? 'change' : 'vote';
  ...
  getQuantileValues(multiplier: number): number[] {
    const quantiles = range(this.numQuantiles * multiplier).map(
      (x) => x / (this.numQuantiles * multiplier)
    );
    const values = this.data
      .map((x) => this.getDemoValue(x))
