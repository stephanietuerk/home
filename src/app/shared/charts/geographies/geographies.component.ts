...
import { extent, geoPath, InternMap, InternSet, map, range, scaleLinear, select, Transition } from 'd3';
...
import { DataGeographyConfig, GeographiesConfig, NoDataGeographyConfig } from './geographies.config';
...
export const GEOGRAPHIES = new InjectionToken<GeographiesComponent>('GeographiesComponent');
@Component({
    // eslint-disable-next-line @angular-eslint/component-selector
    selector: '[data-marks-geographies]',
...
export class GeographiesComponent extends MapContent implements DataMarks, OnChanges, OnInit {
    ...
    setProjection(): void {
        this.projection = this.config.projection.fitSize([this.ranges.x[1], this.ranges.y[0]], this.config.boundary);
    }
    ...
    setValueArrays(): void {
        ...
        this.values.indexMap = new InternMap(this.values.attributeDataGeographies.map((name, i) => [name, i]));
    ...
    initAttributeDataScaleDomain(): void {
        if (this.config.dataGeographyConfig.attributeDataConfig.valueType === 'quantitative') {
            this.setQuantitativeDomainAndBinsForBinType();
        }
        if (this.config.dataGeographyConfig.attributeDataConfig.valueType === 'categorical') {
            this.setCategoricalDomain();
        }
    }

    setQuantitativeDomainAndBinsForBinType(): void {
        if (this.config.dataGeographyConfig.attributeDataConfig.binType === 'equal num observations') {
            this.config.dataGeographyConfig.attributeDataConfig.domain = this.values.attributeDataValues;
        } else if (this.config.dataGeographyConfig.attributeDataConfig.binType === 'custom breaks') {
            ...
            this.config.dataGeographyConfig.attributeDataConfig.numBins =
                this.config.dataGeographyConfig.attributeDataConfig.breakValues.length - 1;
        } else {
            // no bins, equal interval
            let domainValues: any[];
            if (this.config.dataGeographyConfig.attributeDataConfig.domain === undefined) {
            ...
            this.config.dataGeographyConfig.attributeDataConfig.domain = extent(domainValues);
        }

        if (
            // do we need to do this for equal num observations?
            this.config.dataGeographyConfig.attributeDataConfig.binType === 'equal value ranges'
        ) {
    ...
    attributeDataValueFormatIsInteger(): boolean {
        const formatString = this.config.dataGeographyConfig.attributeDataConfig.valueFormat;
        return formatString && typeof formatString === 'string' && formatString.includes('0f');
    }
    ...
    validateNumBinsAndDomainForIntegerValues(): void {
        ...
        const dataRange = [domain[0], domain[domain.length - 1]].map(
            (x) => +formatValue(x, this.config.dataGeographyConfig.attributeDataConfig.valueFormat)
        );
        const numDiscreteValues = Math.abs(dataRange[1] - dataRange[0]) + 1;
        if (numDiscreteValues < this.config.dataGeographyConfig.attributeDataConfig.numBins) {
            this.config.dataGeographyConfig.attributeDataConfig.numBins = numDiscreteValues;
            this.config.dataGeographyConfig.attributeDataConfig.domain = [dataRange[0], dataRange[1] + 1];
        }
    }
    ...
    setCategoricalDomain(): void {
        const domainValues =
            this.config.dataGeographyConfig.attributeDataConfig.domain ?? this.values.attributeDataValues;
        this.config.dataGeographyConfig.attributeDataConfig.domain = new InternSet(domainValues);
    }

    initAttributeDataScaleRange(): void {
        if (this.shouldCalculateBinColors()) {
            const binIndicies = range(this.config.dataGeographyConfig.attributeDataConfig.numBins);
            const colorGenerator = scaleLinear()
                .domain(extent(binIndicies))
                .range(this.config.dataGeographyConfig.attributeDataConfig.colors as any)
                .interpolate(this.config.dataGeographyConfig.attributeDataConfig.interpolator);
            this.config.dataGeographyConfig.attributeDataConfig.range = binIndicies.map((i) => colorGenerator(i));
        } else {
    ...
    initAttributeDataScaleAndUpdateChart(): void {
        let scale;
        if (
            this.config.dataGeographyConfig.attributeDataConfig.valueType === 'quantitative' &&
        ...
        this.zone.run(() => {
            ...
            this.chart.updateAttributeDataConfig(this.config.dataGeographyConfig.attributeDataConfig);
        });
    }
            ...
            .join(
                (enter) =>
                    enter
                        .append('path')
                        .attr('d', this.path)
                        .attr('fill', (d, i, nodes) => this.getConfigFromNode(nodes[i]).fill)
                        .attr('stroke', (d, i, nodes) => this.getConfigFromNode(nodes[i]).strokeColor)
                        .attr('stroke-width', (d, i, nodes) => this.getConfigFromNode(nodes[i]).strokeWidth),
                (update) => update.attr('d', this.path),
                (exit) => exit.remove()
            );
    }
