...
import { extent, group, InternSet, line, map, max, min, range, scaleOrdinal, select, Transition } from 'd3';
...
export class LinesComponent extends XyContent implements XyDataMarks, OnChanges, OnInit {
    ...
    setValueIndicies(): void {
        ...
        this.values.indicies = range(this.values.x.length).filter((i) => domainInternSet.has(this.values.category[i]));
    }

    setScaledSpaceProperties(): void {
        this.zone.run(() => {
            this.chart.updateXScale(this.config.x.scaleType(this.config.x.domain, this.ranges.x));
            this.chart.updateYScale(this.config.y.scaleType(this.config.y.domain, this.ranges.y));
        });
    }
    ...
    canBeDrawnByPath(x: any): boolean {
        return (typeof x === 'number' || this.isDate(x)) && x !== null && x !== undefined;
    }
    ...
    drawMarks(transitionDuration: number): void {
        ...
        if (this.config.lineLabels.display) {
            this.drawLineLabels();
        }
    }
