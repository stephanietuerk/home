export class ValueUtilities {
    static getValueRoundedUpNSignificantDigits(value: number, sigDigits: number): number {
        ...
        if (Math.abs(value) < 1) {
            absRoundedValue = this.getRoundedDecimalLessThanOne(absValueStr, sigDigits);
        } else {
            ...
            const roundedLastSigDigit = Number(firstNDigits[firstNDigits.length - 1]) + 1;
            firstNDigits = firstNDigits.substring(0, firstNDigits.length - 1) + roundedLastSigDigit.toString();
    ...
    private static getRoundedDecimalLessThanOne(valueStr: string, sigDigits: number): number {
        const newValue = [];
